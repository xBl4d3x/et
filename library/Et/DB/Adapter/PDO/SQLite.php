 ...
 */
class DB_Adapter_PDO_SQLite extends DB_Adapter_PDO {

	/**
	 * @var string
	 */
	protected static $_adapter_type = "PDO_SQLite";


	/**
	 * @var DB_Adapter_PDO_SQLite_Config
	 */
	protected $config;

	/**
	 * @param DB_Adapter_PDO_SQLite_Config $config
	 */
	function __construct(DB_Adapter_PDO_SQLite_Config $config){
		parent::__construct($config);
	}


	/**
	 * @return DB_Adapter_PDO_SQLite_Config
	 */
	function getConfig(){
	...
	function truncateTable($table_name) {
		return $this->exec("DELETE FROM " . $this->quoteTableName($table_name));
	}
	...
	function renameTable($source_table_name, $target_table_name) {
		$this->exec("ALTER TABLE {$this->quoteTableName($source_table_name)} RENAME TO {$this->quoteTableName($target_table_name)}");
	}
	...
	function getTableColumnsNames($table_name) {
		$cols = $this->fetchRowsAssociative("PRAGMA table_info({$this->quoteTableName($table_name)});");
