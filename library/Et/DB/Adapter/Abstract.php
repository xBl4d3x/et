...
abstract class DB_Adapter_Abstract extends Object {
	...
	function quoteValue($value){
		...
		throw new DB_Adapter_Exception(
			'Invalid value type \''.gettype($value).'\' to escape/quote - only scalar values, instance of DB_Expression or objects having __toString() or toString() method implemented allowed',
			DB_Adapter_Exception::CODE_QUOTE_FAILED
		);
	 ...
	 * @return bool
	 */
	function fetchKeyExists(DB_Table_Key $key, DB_Query $query = null){
		return $this->fetchRowsCountByKey($key, $query) > 0;
	}
	 ...
	 * @return int
	 */
	function fetchRowsCountByKey(DB_Table_Key $key, DB_Query $query = null){
		$query = $key->getWhereQuery($query);
		return $this->fetchRowsCount($query);
	}
	 ...
	 * @return mixed|bool
	 */
	function fetchSingleValue($sql_query, array $query_data = array(), $value_column = null){
	 ...
	 * @return array
	 */
	function fetchColumnValues($sql_query, array $query_data = array(), $value_column = null){
	 ...
	 * @return int
	 */
	function insertKey(DB_Table_Key $key){
		return $this->insert($key->getTableName(), $key->getColumnValues());
	}
	 ...
	 * @param string|DB_Query|array $where_query [optional]
	...
	function update($table_name, array $new_row_data, $where_query = "", array $where_query_data = array()){
		...
		if($where_query instanceof DB_Query){
			$where_query = $where_query->getWhere()->toSQL($this);
		}
	 ...
	 * @return int
	 */
	function updateRowByKey(DB_Table_Key $key, array $new_row_data, DB_Query $query = null){
		return $this->update($key->getTableName(), $new_row_data, $key->getWhereQuery($query));
	}


	/**
	 * @param string $table_name
	 * @param string|array|DB_Query $where_query
	 * @param array $where_query_data [optional]
	 *
	 * @return int
	 * @throws DB_Adapter_Exception
	 ...
	 * @return int
	 */
	function deleteRowsByKey(DB_Table_Key $key, DB_Query $query = null){
		return $this->delete($key->getTableName(), $key->getWhereQuery($query));
	}
	 ...
	 * @return int
	 */
	public function fetchRowsCount($sql_query, array $query_data = array(), $ignore_query_limit_and_offset = true){
		...
		if($sql_query instanceof DB_Query){
			...
			return (int)$this->fetchSingleValue($cloned_query);
		}
		...
		return $this->fetchSingleValue($sql_query, $query_data);
	}
	 ...
	 * @return int
	 */
	abstract function copyTableColumns($source_table, $target_table, array $columns, $where_query = "", array $where_query_data = array());
	 ...
	 * @param int $padding_offset [optional]
	 * @return string
	 */
	abstract function buildQuery(DB_Query $query, $padding_offset = 0);
	...
	function createTable(DB_Table_Definition $table_definition, $drop_if_exists = false, $rename_to_if_exists = null){
		...
		return $this->exec($this->getCreateTableQuery($table_definition));
	}
